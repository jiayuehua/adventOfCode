...
private:
  ...
  static inline constexpr int TreeSZ = 8;
  //static inline constexpr int NX = 5;
  //static inline constexpr int NY = 11;
  //static inline constexpr int TreeSZ = 5;
...
public:
  ...
  int shortestpath()
  {
    FixedMatrix<int, TreeSZ, TreeSZ> adjmatrix;
    std::vector<Mypair> adjp;
    for (int i = 0; i < TreeSZ - 1; ++i) {
      for (int j = i + 1; j < TreeSZ; ++j) {
        adjp.push_back({ i, j });
      }
    }
    std::for_each(std::execution::par_unseq, adjp.begin().adjp.end(), [this, &adjmatrix](Mypair p) {
      adjmatrix(p.x, p.y) = shortestpath(this->trees_[p.x], this->trees_[p.y]);
    ...
    for_each_permutation(trees_.begin(), trees_.end() - 1, trees_.end() - 1, f);
    return totallen;
  }
  friend std::istream &operator>>(std::istream &is, Game &g)
  {
    ...
    ranges::rotate(g.trees_, g.trees_.begin() + 1);
    return is;
  }
