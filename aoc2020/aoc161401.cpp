...
int main(int argc, char **argv)
{
  ...
  std::transform(std::execution::par_unseq, v.begin(), v.end(), vs.begin(), [](int i) {
    auto sum = cx::md5_new(("cuanljph" + std::to_string(i)).c_str());
  ...
  auto validindexs = vs | views::take(N) | views::filter(valid) | views::take(64) | views::drop(63);
