...
x3::rule<class factor, int64> const factor = "factor";
...
auto const expr_def = term[indentity] >> *(('*' >> term[multi]));//左结合
auto const term_def = factor[indentity] >> *(('+' >> (factor)[add]) | ('-' >> (factor)[sub]));//左结合
